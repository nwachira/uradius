...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['routername'])) {
    ...
    $routerName = 'Router1'; // Hard-coded value
        ...
        if ($router['name'] === $routerName) { // Check if router name matches POSTed routername
            ...
            foreach ($plans_hotspot as $plan) {
                if ($router['name'] == $plan['routers']) {
                    ...
                    $paymentlink = "https://unganishanetworks.com/phpnuxbill/index.php?_route=plugin/hotspot_pay&routerName={$router['name']}&planId={$plan['id']}&routerId={$router['id']}";
